...
写作时 `ragEnabled=true`（默认）会自动将相关片段注入 plot / 正文 prompt。每章 / 剧情步骤完成后增量更新索引。

### 选章重写

Web 阅读器 → 选中段落 →「重写本章」，或 API：

```bash
curl -s -X POST http://127.0.0.1:18082/novels/default/<novel>/chapters/3/regenerate \
  -H 'Content-Type: application/json' \
  -d '{"instruction":"加强开篇悬念，语气更冷峻","layer":"chapter"}'
```
...
```bash
export RAG_EMBEDDING_BASE_URL="${AI_BASE_URL}"   # 默认同 AI_BASE_URL
export RAG_EMBEDDING_API_KEY="${AI_API_KEY}"
export RAG_EMBEDDING_MODEL="text-embedding-3-small"
```

工作区会额外生成 `rag/vectors.json`。
...
| `ragSearchMode` | `hybrid` | 检索模式：`hybrid` / `vector` / `bm25`（无 embedding 时自动回退 bm25） |
| `ragTopK` | `5` | 每次检索返回片段数 |
