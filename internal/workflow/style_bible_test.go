...
func TestTeamTemplateHasStyleBibleStep(t *testing.T) {
	...
	foundOutline, foundRefine, foundPlots, foundBible, foundChapters := false, false, false, false, false
	for _, step := range spec.Steps {
		...
		case "chapters":
			...
			if len(step.DependsOn) < 2 {
				t.Fatalf("chapters should depend on plots+bible: %v", step.DependsOn)
			}
		}
	}
	if !foundOutline || !foundRefine || !foundPlots || !foundBible || !foundChapters {
		t.Fatalf("steps missing: outline=%v refine=%v plots=%v bible=%v chapters=%v", foundOutline, foundRefine, foundPlots, foundBible, foundChapters)
	}
